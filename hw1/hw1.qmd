...
Explanation of Code: To improve model fit, we perform an exhaustive search to identify the best-fitting linear regression model for predicting the Fertility variable in the swiss dataset by testing all possible combinations of five different transformations (identity, log, inverse, B-splines, and orthogonal polynomials) applied to each of the five predictor variables (Agriculture, Examination, Education, Catholic, and Infant.Mortality). For each unique combination of transformations (a total of 5⁵ = 3,125 combinations), it dynamically constructs a model formula, fits a linear model using lm(), and evaluates the model based on the AIC value. If the current model achieves a lower AIC than any previously evaluated model, it is stored as the best model. At the end of the loop, we will obtain the best model with the lowest AIC value, along with the corresponding transformation combination for each predictor variable. 

```{r}
predictors <- c("Agriculture", "Examination", "Education", "Catholic", "Infant.Mortality")
transformations <- c("identity", "log", "inv", "bs", "poly")

transform_var <- function(var, trans) {
  switch(trans,
         identity = var,
         log = log(var + 1e-6), 
         inv = 1 / (var + 1e-6),
         bs = bs(var, df = 4),
         poly = poly(var, degree = 2, raw = FALSE))
}

generate_model_formula <- function(trans_combo) {
  terms <- mapply(function(var, trans) {
    if (trans == "identity") {
      var
    } else if (trans %in% c("log", "inv")) {
      paste0("I(", trans, "(", var, " + 1e-6))")
    } else if (trans %in% c("bs", "poly")) {
      paste0(trans, "(", var, ", df=4)")
    }
  }, predictors, trans_combo, SIMPLIFY = TRUE)
  
  formula_str <- paste("Fertility ~", paste(terms, collapse = " + "))
  as.formula(formula_str)
}

combos <- expand.grid(rep(list(transformations), length(predictors)),
                      stringsAsFactors = FALSE)
best_aic <- Inf  # Initialize with worst possible AIC
best_transformed_model <- NULL
best_combo <- NULL

for (i in 1:nrow(combos)) {
  trans_combo <- combos[i, ]
  formula <- generate_model_formula(trans_combo)
  
  model <- tryCatch(
    lm(formula, data = swiss),
  ...
  if (!is.null(model)) {
    ...
    if (current_aic < best_aic) {
      ...
      best_transformed_model <- model
      best_combo <- trans_combo
    }
  }
}

cat("Best AIC:", best_aic, "\n")
cat("Best transformation combo:\n")
for (j in 1:length(predictors)) {
  cat(predictors[j], ":", best_combo[[j]], "\n")
}
```
```{r}
summary(best_transformed_model)
```

We will conduct stepwise AIC selection for the model that we obtained above (with the transformed predictor variables) to see if another subset of these variables will create a better model fit. 

```{r}
swiss$bs_exam1 <- bs(swiss$Examination, df=4)[,1]
swiss$bs_exam2 <- bs(swiss$Examination, df=4)[,2]
swiss$bs_exam3 <- bs(swiss$Examination, df=4)[,3]
swiss$bs_exam4 <- bs(swiss$Examination, df=4)[,4]

swiss$poly_edu1 <- poly(swiss$Education, 4)[,1]
swiss$poly_edu2 <- poly(swiss$Education, 4)[,2]
swiss$poly_edu3 <- poly(swiss$Education, 4)[,3]
swiss$poly_edu4 <- poly(swiss$Education, 4)[,4]

swiss$log_infant <- log(swiss$Infant.Mortality + 1e-6)

reduced_transformed_model <- lm(Fertility ~ Agriculture + Catholic + log_infant +
                   bs_exam1 + bs_exam2 + bs_exam3 + bs_exam4 +
                   poly_edu1 + poly_edu2 + poly_edu3 + poly_edu4,
                 data = swiss)

step_model <- stepAIC(reduced_transformed_model, direction = "both", trace = FALSE)

cat("AIC of the best reduced transformed model:", AIC(reduced_transformed_model), "\n")
cat("Predictors in the best reduced transformed model:\n")
print(attr(terms(reduced_transformed_model), "term.labels"))
summary(reduced_transformed_model)
```

These results show that the full transformed model already has the lowest possible AIC among all nesed models considered by stepwise selection. 

```{r}
AIC(full_model, best_transformed_model, reduced_transformed_model)
```

The AIC score of the full transformed model is lower than that of the full model (before transforming the predictors). The full_model has AIC value of 323.38 while the best_transformed_model has an AIC score of 312.0989. Therefore, we can conclude that currently, the best model can be characterized by: 

$$ Fertility = 
  14.85392 
  - 0.16796 * Agriculture 
  - 27.98577 * bs(Examination, df=4)[,1] 
  + 6.48764 * bs(Examination, df=4)[,2] 
  - 51.49237 * bs(Examination, df=4)[,3] 
  + 21.05979 * bs(Examination, df=4)[,4] 
  - 86.49956 * poly(Education, 4)[,1] 
  - 37.36313 * poly(Education, 4)[,2] 
  + 9.59323 * poly(Education, 4)[,3] 
  + 9.10119 * poly(Education, 4)[,4] 
  + 0.09798 * Catholic 
  + 25.18993 * log(Infant.Mortality + 1e-6) $$
...
```{r}
par(mfrow = c(2, 2))
plot(best_transformed_model)
par(mfrow = c(1, 1))
```
The results of the Shapiro Wilk normality test are shown below. The p-value is 0.8367. Therefore, we fail to reject the null hypothesis that the residuals are normally distributed. Therefore, this suggests that the model's residuals do not significantly deviate from normality. 

```{r}
shapiro.test(residuals(best_transformed_model)) 
```
Lastly, we will run the Studentized Breusch-Pagan test to check for heteroscedasticity (non-constant variance of residuals) in a linear regression model. Because the residual plots from above hinted at heteroscedastcity, we will run this test to ensure that the residuals have constant variance. The p-value resulting from the Breusch-Pagan Test is 0.7476 so we fail to reject the null hypothesis. Therefore, we are able to conclude that the residuals have constant variance. 

```{r}
bptest(best_transformed_model) 
```
...
new_data <- data.frame(
  ...
  Education = c(12, 8),
  Catholic = c(60, 95),
  Infant.Mortality = c(20, 24)
)

bs_exam_basis <- bs(swiss$Examination, df = 4)
poly_edu_basis <- poly(swiss$Education, degree = 4, raw = FALSE)

attr(bs_exam_basis, "class") <- "bs"
attr(poly_edu_basis, "class") <- "poly"

bs_exam_pred <- predict(bs(swiss$Examination, df = 4), new_data$Examination)
poly_edu_pred <- predict(poly(swiss$Education, degree = 4, raw = FALSE), new_data$Education)

new_data$bs_exam1 <- bs_exam_pred[,1]
new_data$bs_exam2 <- bs_exam_pred[,2]
new_data$bs_exam3 <- bs_exam_pred[,3]
new_data$bs_exam4 <- bs_exam_pred[,4]

new_data$poly_edu1 <- poly_edu_pred[,1]
new_data$poly_edu2 <- poly_edu_pred[,2]
new_data$poly_edu3 <- poly_edu_pred[,3]
new_data$poly_edu4 <- poly_edu_pred[,4]

new_data$log_infant <- log(new_data$Infant.Mortality + 1e-6)
predicted_fertility <- predict(best_transformed_model, newdata = new_data)
print(predicted_fertility)
```

We are able to see that the first observation has a predicted fertility of 76.76199. The second observation has a predicted pfertility of 99.87283. 
...
Like many underdeveloped countries in the late 19th century, Switzerland in 1888 was undergoing a demographic transition—a shift from high fertility and mortality rates to lower levels as a result of social and economic development. Understanding the effects of different socioeconomic and cultural factors on fertility during this period highlights the importance of using statistical modeling to better understand how different and therefore improve the of countries in our current society. 

  + OBJECTIVE: state the overall purpose of your research, e.g., what kind of knowledge gap you are trying to fill in: 

The goal of this analysis is to examine how various socio-economic and religious factors—such as Agriculture, Examination (education access), Education, Catholicism, and Infant Mortality—are associated with fertility rates in Switzerland's provinces during the late 19th century. We aim to identify the most relevant predictors (and its interactions) of fertility and assess how model transformations can improve the model fit. The development of such a model can help future global leaders to better understand 
...
Next, we check whether the predictor variable diastolic blood pressure is significant in the regression model. The regression model that is utilized in this question is the full model before removing the triceps and insulin. The p-value corresponding to diastolic blood pressure is 0.90446 which is not significant in the full model. This means that DBP does not contribute significantly to the regression model when other variables are included. 
