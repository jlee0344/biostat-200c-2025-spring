...
```{r}
county_count <- read_csv("~/Downloads/04-04-2020.csv.gz") %>%
...
```{r}
county_info <- read_csv("~/Downloads/us-county-health-rankings-2020.csv.gz") %>%
  filter(!is.na(county)) %>%
  mutate(fips = as.numeric(fips)) %>%
  dplyr::select(fips, 
         state,
...
```{r}
county_data %>%
  dplyr::mutate(cfr = deaths / confirmed) %>%
...
```{r}
write_csv(county_data, "~/Downloads/covid19-county-data-20200404.csv.gz")
```
### Note:
...
```{r}
county_data <- read_csv("~/Downloads/covid19-county-data-20200404.csv.gz") %>%
  print(width = Inf)
```
...
top_5_predictors <- head(significant_predictors, 5)
print(top_5_predictors)
```

The top 3 most signficiant predictors and their corresponding interpretations are: 

1. **percent_homeowners**: (Regression coefficient: -0.03398152 with a p-value of 3.217538e-08. Interpretation: A one percent increase in the homeownership rate is associated with a 0.03398152 decrease in the log-odds of COVID-19 deaths, holding all other predictors constants. In other words, a one percent increase in the homeownership rate is associated with a decrease in the odds of COVID-19 deaths by 3.3 percent, holding all other predictors constant. We are able to say that counties with high homeownership rates showed significantly lower COVID-19 mortality. 

2. **percent_severe_housing_problems**: (Regression coefficient: -0.04473908) with a p-value of 3.207828e-05. Interpretation: A one percent increase in severe housing problems is associated with a 0.04473 decrease in the log-odds of COVID-19 deaths, holding all other predictors constant. In other words, a one percent increase in the severe housing problems is associated with a decrease in the odds of COVID-19 deaths by 4.37 percent, holding all other predictors constant. We are able to say that counties with high severe housing problems showed significantly lower COVID-19 mortality. 
...
The two key lambda values that are calculated are: 
1. lambda.min: Minimizes prediction error 
2. lambda.1se: Largest lambda value withiin 1 standard error of the minimum (simpler model) 

```{r}
lambda_min <- cv_lasso$lambda.min
lambda_1se <- cv_lasso$lambda.1se

cat("Lambda (min error):", lambda_min, "\n")
cat("Lambda (1se rule):", lambda_1se)
```

1. Predictors from the Most Accurate Model (using lambda.min) 

```{r}
coef_min <- coef(cv_lasso, s = "lambda.min")
selected_vars_min <- rownames(coef_min)[which(coef_min != 0)][-1]  
print(selected_vars_min)
```

2. Predictors from the Simpler Model (using lambda.1se) 

```{r}
coef_1se <- coef(cv_lasso, s = "lambda.1se")
selected_vars_1se <- rownames(coef_1se)[which(coef_1se != 0)][-1]
print(selected_vars_1se)
```

The model with LASSO regularization (from both lambda values) comes out to have no predictor variables besides the intercept. Therefore, we refit the binomial logistic regression model with no predictors. It is possible the LASSO regression did not select any of the predictors for the logistic model because LASSO's penalty term (lambda values) were so strong that it forced all the predictor coefficients to exactly 0 (extreme regularization). In addition, it is also possible that one or more of the predictors perfectly predict the outcome, making the logisitc regression unstable. For future exploration, it is vital that we check data issues including perfect separation and scaling, reassess predictor relevance, and try weaker penalties or alternative methods. 
...
The AIC score of the model with only intercepts is 5141.6 which is significantly higher than that of the the sub-model chosen from the stepwise selection AIC method. 
