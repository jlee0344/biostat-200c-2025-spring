...
The table below showcases the 5 most significant predictors as a result of running the binomail logistic regression: 
...
```{r}
# 1. Perform Analysis of Deviance (Type II Wald tests)
anova_results <- Anova(model_1, type = 2)
```
...
kable(top_predictors, digits = c(NA, 2, 2, 4), 
      caption = "Top 10 Most Significant Predictors of County-Level COVID-19 Deaths")
```
...
confint(model_1) 
coef_estimates <- summary(model_1)$coefficients
wald_ci <- cbind(
  Estimate = coef_estimates[, "Estimate"],
  Lower_95_CI = coef_estimates[, "Estimate"] - 1.96 * coef_estimates[, "Std. Error"],
  Upper_95_CI = coef_estimates[, "Estimate"] + 1.96 * coef_estimates[, "Std. Error"]
)
print(wald_ci)
