...
**2. Linearity of Log-Odds (Linearity Assumption): ** The relationships between the predictors (such as percent_smokers, percent_fair_or_poor_health) and the log-odds of COVID-19 outcomes may not be linear. 
...
**5. Binary Outcome Requirement: ** Logistic regression requires a binary outcome for the "deaths" and "confirmed" predictor variables. Therefore, we must check that these variables only hold 2 values (filter out NA values). 

**6. Missing Data **: Counties with missing health/demographic data may be excluded, biasing results. 
...
**Solution ** 
We will run a logisitc regression where the outcome is a binary variable "death_binary" that will have the value 1 if the number of deaths in a county is greater than the median number of deaths overall and 0 if the number of deaths in a county is less than the median number of deaths overall. The logistic regression model will esimate how each predictor affects the log-odds of a county having above-median deaths. 

```{r}
county_data <- county_data %>%
  mutate(death_binary = as.numeric(deaths > median(deaths, na.rm = TRUE)))
```
...
predictors <- setdiff(names(county_data), c("death_binary", excluded_predictors))

formula <- as.formula(
  paste("death_binary ~", paste(predictors, collapse = " + "))
)
...
**Solution ** 
The regression coefficients of 3 significant predictors with p-value that is less than 0.05 are: 

1. **percent_rural**: (Regression coefficient: -1.302e-02) with a p-value of 0.01874. Interpretation: A one percent increase in rural population is associated with a -1.302e-02 decrease in the log-odds of high deaths in a county, holding all other predictors constants. In other words, a one percent increase in rural population is associated with a 0.987 or 1.29 percent decrease in the odds of high deaths in a county, holding all other predictors constant. 

2. **population_2**: (Regression coefficient: 5.392e-06) with a p-value of 1.11e-09. Interpretation: 

3. **percent_2**: (Regression coefficient: )
...
1. Goodness of Fit Test (Comparison of the Current Model versus Saturated Model): We will test whether the logisitc regression model fits the data adequately by comparing the residual deviance to a chi-squared distribution. In other words, the goodness of fit tests evaluates how well a statistical model or distribution fits a set of observed data. The p-value comes out to be 0.143 which is greater than 0.05. Therefore, we have sufficient evidence to fail to reject the null hypothesis and conclude that the model is acceptable and fits well to the given dataset. 

```{r}
p_value_gof <- 1 - pchisq(model_1$deviance, df = model_1$df.residual)
cat("Goodness-of-fit p-value:", p_value_gof, "\n")
```

2. Likelihood Ratio Test (Comparison of the Current Model versus Intercept-Only Model): We will test whether the logistic regression model is significantly better than the intercept-only model. In other words, we will evaluate whether the predictors collectively improve the model over a baselien (intercept-only) model. The p-value comes out to be < 2.2e-16 which is. less than 0.05. Therefore, we can conclude that our current logisitic regression model with the included predictors significantly improves the model over a baseline (intercept-only) mdoel. 


```{r}
null_model <- glm(death_binary ~ 1, data = county_data, family = binomial)
anova(null_model, model_1, test = "LRT")  
```
...
For the simple logistic model gi
$$
